...
###########################################################################
#                                                                         #
# Title: Analysis of data files: from 0 to the results to publish.        #    
# Author: Rosa M. Maza Quiroga (rosa.mmq3290@gmail.com)                   #
# Date: March 2019                                                        #
#                                                                         #
###########################################################################
...
# Funtcion to install all libraries automatically
InstallPack <- function(pack){ 
...
# Install libraries
InstallPack("devtools")
InstallPack("factoextra")
InstallPack("ggpubr")
InstallPack("gridExtra")
InstallPack("kohonen")
InstallPack("lawstat")
InstallPack("lsmeans")
InstallPack("nortest")
...
InstallPack("xlsx")
InstallPack("vcd")
InstallPack("verification")
InstallPack("visreg")

# The obsoleted packege is installed manually
...
### 2.- Analysis of the distribution
...
### Analysis of relationship between a cuantitative variable and a cualitative variable  
#
# Thus, this was postulated:  
#   
#   - H0: variables are independent.  
#   - HA: variables are dependent.  
# 
# If p-value is “small”, (less than 0.05), we can then reject the null hypothesis. This is because reject H0 if it’s true has small risk.  


## 2.2.-  Normality analysis: Kolmogorov-Smirnov with Lilliefors correction

### Normality test: Kolmogorov-Smirnov with Lilliefors correction  
#
# - H0: the distributions are equals and normals.  
# - HA: the distributions are not equals so the study distribution do not follow a normal distribution.  
# 
# We use Kolmogorov-Smirnov with Lilliefors correction, *lillie.test()* function.  
# 
...
# 
# *Note: we can use Shapiro-Wilk test, but this test does not operates well with repeated data. 


## 2.3.-  Comparison of distributions: T-test

### Parametric test: T test  
# 
# - H0: the means are equals. Thus variable is independant of the group to wich it belong.  
# - HA: te means are differents. Then variable dependant of the group to wich it belong.  
# 
# When is made T test, we have to indicate the variance (similar or different). For that we do the homocedasticity proof.


## 2.4.- Comparison of distributions: Wilcoxon-test 

### Non parametric test: Mann-Whitney-Wilcoxon rank sum test (or test U)    
# 
# - H0: the medians are equals. Thus variable is independant of the group to wich it belong.  
# - HA: te medians are differents. Then variable dependant of the group to wich it belong.  
# 
# *Note:  
#   - U mann-whitney: wilcox.test() function for independant data.  
#   - wilcoxon: wilcolx.exact() function for paired data. 


## 2.4.- Comparison of distributions: ChiSquare-test 
...
# - HA: te medians are differents. Then variable dependant of the group to wich it belong.  
# 
# We use *chisq.test()* function.  
# Function parameters: if <simulate.p.value> = FALSE, the p-value is computed from the asymptotic chi-squared distribution of the test statistic; 
# continuity correction is only used in the 2-by-2 case (if correct is TRUE, the default).
...
# EXAMPLES: a) BMI variable (t.test)   ,   b) Age variable (wilcoxon.test)     and     c) Sex variable (chisq.test)
...
### 3.- Analysis of variance/covariance

# ### ANCOVA hypothesis  
# 
# - H0: there is no relationship between cofactors and dependent variable.  
# - HA: ther is relationship between cofactors and dependent variable.  
# 
#   
#   ### Notes:
#   
# - We can run a model as either an ANOVA or a regression, we have selected ANOVA model with aov function.    
# 
# - When we construct the model, we use some covariables. Their order does not matter, we get similar results independent of the order of cofactors. Square type seleccted are Type III.
# 
# - The intercept is the estimate of the dependent variable when all the independent variables are 0. 



# EXAMPLES: a) Molecule1   ,   b) Molecule2     and     c) Molecule3
...
### 6.- Clustering (non supervised machine learning)
